...
func (rt *_router) getComments(w http.ResponseWriter, r *http.Request, ps httprouter.Params, ctx reqcontext.RequestContext) {
	...
	dbposterID, err := rt.db.CheckImagePoster(photoid, username)
	if err != nil {
	...
	c, errC := rt.db.CheckBanned(dbposterID, dbuserAuth.ID)
	if errC != nil {
		// In this case, we have an error on our side. Log the error (so we can be notified) and send a 500 to the user
		// Note: we are using the "logger" inside the "ctx" (context) because the scope of this issue is the request.
	...
	dblistComment, errLike := rt.db.GetComments(photoid, dbuserAuth.ID)
	if errLike != nil {
		// In this case, we have an error on our side. Log the error (so we can be notified) and send a 500 to the user
		// Note: we are using the "logger" inside the "ctx" (context) because the scope of this issue is the request.
		ctx.Logger.WithError(err).Error("can't get the comments")
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	_ = json.NewEncoder(w).Encode(dblistComment)
}
